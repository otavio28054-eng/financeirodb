...
function buildSel(elId,selKey='outros'){
  ...
  el.innerHTML=groups.map(g=>
    `<optgroup label="${g.label}">${g.cats.map(c=>`<option value="${c.k}"${c.k===selKey?' selected':''}>${c.l}</option>`).join('')}</optgroup>`
  ).join('');
}
...
let viewY=todayDate.getFullYear(), viewM=todayDate.getMonth();
let annualYear=todayDate.getFullYear();
...
buildSel('qCat','mercado');
buildSel('fCat','outros');

// ── DATA ───────────────────────────────────────────────────────────────
// fixed: repeat every month
// card: parceladas com startMonth+pa+pt; fixas sem fim
// months[key]: {income:[],variable:[]}
// investments: [{id,name,value,month,ts}]

function loadD(){
  try{
    const d=JSON.parse(localStorage.getItem('fin6')||'null');
    if(d&&d.fixed&&d.months)return d;
  }catch(e){}
  ...
  return{
    ...
    months:{
      [cm]:{
        income:[
          {id:'i1',name:'Salário dia 15',value:1367.28,cat:'salario',date:mKey(todayDate.getFullYear(),todayDate.getMonth())+'-15',ts:Date.now()-200},
          {id:'i2',name:'Salário dia 30',value:1538.95,cat:'salario',date:mKey(todayDate.getFullYear(),todayDate.getMonth())+'-30',ts:Date.now()-100},
        ],
        variable:[
...
function cardInfo(card,y,m){
  ...
  const cur=y*12+m-(sy*12+(sm-1))+1;
  return `${cur}/${card.pt}`;
}

// end month calculator
function addMonths(startMonth,pa,pt){
  if(!startMonth||!pa||!pt)return null;
  const[sy,sm]=startMonth.split('-').map(Number);
  const remaining=pt-pa;
  let ey=sy,em=sm+remaining;
...
function goTab(t){
  document.querySelectorAll('.tab').forEach((el,i)=>{
    el.classList.toggle('active',['quick','detail','charts','annual','invest'][i]===t);
  });
  document.querySelectorAll('.tab-content').forEach(el=>el.classList.remove('active'));
  document.getElementById('tab-'+t).classList.add('active');
  curTab=t;
  if(t==='charts')setTimeout(renderCharts,50);
  if(t==='annual'){annualYear=viewY;setTimeout(renderAnnual,50);}
  if(t==='invest'){renderInvStats();setTimeout(renderInvChart,50);}
}
...
function updateDivPreview(){
  ...
  if(on&&!isNaN(v)&&v>0){
    ...
    prev.textContent='Valor dividido: '+fmt(v/2)+' (metade de '+fmt(v)+')';
  }else{
...
function calcEndMonth(){
  ...
  const startMonth=mKey(viewY,viewM);
  const end=addMonths(startMonth,pa,pt);
...
function quickAdd(){
  ...
  const rawValue=parseFloat(document.getElementById('qVal').value);
  const divOn=document.getElementById('qDivCheck').checked;
  const value=divOn ? parseFloat((rawValue/2).toFixed(2)) : rawValue;
  ...
  if(parOn){
    ...
    md.income.unshift({id:uid(),name,value,cat,date,ts:Date.now(),split:divOn});
  } else if(c&&c.t==='investment'){
    D.investments.push({id:uid(),name,value,month:mKey(viewY,viewM),ts:Date.now(),split:divOn});
  } else {
    const md=getMonth(viewY,viewM);
    md.variable.unshift({id:uid(),name,value,cat,date,ts:Date.now(),split:divOn});
  }
  ...
  document.getElementById('qPA').value='';
  document.getElementById('qPT').value='';
...
function renderQuick(){
  ...
  el.innerHTML=all.map(item=>{
    ...
    return `<div class="qitem">
      ...
      <div class="qitem-val" style="${vc}">${sign} ${fmt(item.value)}</div>
      <button class="qitem-del" onclick="delQuick('${item.id}','${list}')">✕</button>
    </div>`;
  }).join('');
}

function delQuick(id,list){
  const md=getMonth(viewY,viewM);
  md[list]=md[list].filter(x=>x.id!==id);
  persist();renderAll();toast('Removido.');
}
...
function renderDetail(){
  ...
  if(!md.income.length)incEl.innerHTML='<div class="empty">nenhuma renda lançada</div>';
  else incEl.innerHTML=md.income.map(item=>`<div class="iitem">
    <div class="cdot" style="background:var(--green)"></div>
    <div class="iitem-main">
      <div class="iitem-name">${item.name}</div>
      ${item.date?`<div class="iitem-sub">${fmtDate(item.date)}</div>`:''}
    </div>
  ...
  if(!D.fixed.length)fixEl.innerHTML='<div class="empty">nenhuma despesa fixa</div>';
  else fixEl.innerHTML=D.fixed.map(item=>{
    const c=catMap[item.cat]||{c:'#888',l:'?'};
    return `<div class="iitem">
      <div class="cdot" style="background:${c.c}"></div>
      <div class="iitem-main">
        <div class="iitem-name">${item.name}</div>
        <div class="iitem-sub">${c.l}${item.day?' · dia '+item.day:''}</div>
      </div>
  ...
  if(!cards.length)cardEl.innerHTML='<div class="empty">nenhuma parcela ativa</div>';
  else cardEl.innerHTML=cards.map(item=>{
    const c=catMap[item.cat]||{c:'#888',l:'?'};
    ...
    return `<div class="iitem">
      ...
      <div class="iitem-main">
        <div class="iitem-name">${item.name}</div>
        <div class="iitem-sub">${c.l} · ${info}${end?' · até '+end:''}</div>
      </div>
  ...
  // VARIABLE — grouped by cat, ranked
  ...
  md.variable.forEach(item=>{
    ...
    groups[item.cat].items.push(item);
  ...
  if(!sorted.length)html='<div class="ilist"><div class="empty">nenhum gasto variável — use o lançamento rápido</div></div>';
    ...
    html+=`<div class="vcat-block">
      ...
      <div class="vcat-items">
        ${g.items.sort((a,b)=>b.value-a.value).map(item=>`<div class="iitem">
          <div class="cdot" style="background:${g.cat.c}"></div>
          <div class="iitem-main">
            <div class="iitem-name">${item.name}</div>
            ${item.date?`<div class="iitem-sub">${fmtDate(item.date)}</div>`:''}
...
function toggleVcat(hd){
  const it=hd.nextElementSibling;
  const ch=hd.querySelector('.vcat-chev');
  if(!it)return;
  const hidden=it.classList.toggle('hidden');
...
function openNew(defaultCat,type='variable'){
  ...
  document.getElementById('fName').value='';document.getElementById('fVal').value='';
  document.getElementById('fDate').value=todayStr();
  document.getElementById('fDay').value='';
  document.getElementById('fPA').value='';document.getElementById('fPT').value='';
...
function openEdit(id,list){
  const md=getMonth(viewY,viewM);
  const item=md[list].find(x=>x.id===id);
  if(!item)return;
  mCtx={type:list,id};
  document.getElementById('mTitle').textContent='Editar lançamento';
  document.getElementById('fName').value=item.name;
  document.getElementById('fVal').value=item.value;
  document.getElementById('fDate').value=item.date||todayStr();
  buildSel('fCat',item.cat);
  document.getElementById('fDayW').style.display='none';
  document.getElementById('fCatW').style.display='';
...
function tglPar(){
  document.getElementById('parRow').style.display=document.getElementById('fTipo').value==='parcelada'?'':'none';
  calcModalEnd();
}
...
function addInv(){
  ...
  persist();renderAll();
  if(curTab==='invest'){renderInvStats();setTimeout(renderInvChart,50);}
  toast('Aporte registrado!');
}

function renderInvStats(){
  const all=D.investments;
  ...
  if(last){
    document.getElementById('invLst').textContent=fmt(last.value);
    const[y,m]=last.month.split('-');
    document.getElementById('invLstD').textContent=MESES[parseInt(m)-1]+' '+y;
...
function delInv(id){D.investments=D.investments.filter(x=>x.id!==id);persist();renderAll();if(curTab==='invest'){renderInvStats();setTimeout(renderInvChart,50);}toast('Removido.');}

// ── CHARTS — safe destroy+recreate ────────────────────────────────────
function safeChart(id,cfg){
  // Destroy any existing chart on this canvas via Chart.js registry
...
function renderCharts(){
  ...
  const dV=[fix,card,...srt.map(g=>parseFloat(g.total.toFixed(2)))];
  ...
  if(dV.some(v=>v>0)){
    ...
    document.getElementById('donutLeg').innerHTML=dL.map((l,i)=>
      `<div class="leg-i"><div class="leg-dot" style="background:${dC[i]}"></div><span>${l}: ${fmt(dV[i])}</span></div>`
    ).join('');
  }else{
    document.getElementById('donutWrap').innerHTML='<div class="chart-empty">sem dados neste mês</div>';
  ...
  if(srt.length){
    safeChart('cBar',{
      type:'bar',
      data:{
        labels:srt.map(g=>g.cat.l.length>13?g.cat.l.slice(0,13)+'…':g.cat.l),
        datasets:[{data:srt.map(g=>parseFloat(g.total.toFixed(2))),backgroundColor:srt.map(g=>g.cat.c+'99'),borderColor:srt.map(g=>g.cat.c),borderWidth:1,borderRadius:5}]
      },
      options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,
        plugins:{legend:{display:false},tooltip:{callbacks:{label:ctx=>' '+fmt(ctx.parsed.x)}}},
        scales:{
          x:{grid:{color:'rgba(255,255,255,.04)'},ticks:{color:'#5a6a5a',font:{family:'DM Mono',size:9},callback:v=>'R$'+v.toLocaleString('pt-BR')}},
          y:{grid:{color:'rgba(255,255,255,.04)'},ticks:{color:'#8a9a8a',font:{family:'DM Mono',size:9}}}
        }}
    });
  }else{
    document.getElementById('barWrap').innerHTML='<div class="chart-empty">sem gastos variáveis</div>';
  }
  ...
  safeChart('cFlow',{
    ...
    options:{responsive:true,maintainAspectRatio:false,
      ...
      scales:{
        x:{grid:{color:'rgba(255,255,255,.04)'},ticks:{color:'#5a6a5a',font:{family:'DM Mono',size:9},autoSkip:false,maxRotation:45}},
...
function renderInvChart(){
  const byM={};
  D.investments.forEach(x=>{byM[x.month]=(byM[x.month]||0)+x.value;});
  const months=Object.keys(byM).sort();
  if(!months.length)return;
  let acc=0;
  const cumul=months.map(m=>{acc+=byM[m];return parseFloat(acc.toFixed(2));});
  safeChart('cInv',{
    type:'line',
    data:{
...
function renderAnnual(){
  ...
  grid.innerHTML=MESES.map((name,m)=>{
  ...
  safeChart('cAnnual',{
    ...
    data:{
      ...
      datasets:[{
        label:'Saldo',data:bals,
        borderColor:'#4ade80',backgroundColor:'rgba(74,222,128,.07)',
        borderWidth:2,pointRadius:4,
        pointBackgroundColor:bals.map(v=>v>=0?'#4ade80':'#f87171'),
        fill:true,tension:.3
      }]
    },
    options:{responsive:true,maintainAspectRatio:false,
...
function renderAll(){
  ...
  if(curTab==='annual')setTimeout(renderAnnual,50);
}
